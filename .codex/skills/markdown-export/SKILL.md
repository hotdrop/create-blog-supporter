...
- Markdown出力タスクでは、出力先、共有方法、ファイル名規則をAcceptance Criteriaに含める。
- MVPではDownloads保存やStorage Access Frameworkを既定採用せず、必要な場合はユーザーに相談する。
...
## 共有ルール
- FileProviderを使う場合は、公開パスを `exports` 配下など成果物共有用の限定領域にする。
- 共有Intentには `FLAG_GRANT_READ_URI_PERMISSION` を付与し、受け取り側へ一時的な読み取り権限だけを渡す。
- ユーザーの取得導線は保存完了表示だけで終えず、共有シートなど実際に端末外へ渡せる手段まで確認する。
...
- 出力先、共有方法、ファイル名規則、取得導線がAcceptance Criteriaどおりに実装されている。
